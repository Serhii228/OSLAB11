        ...
        static void Main(string[] args)
        {
            while (true)
            {
                if(Math.Abs(timeRates.Day - DateTime.Now.Day) == 0)
                {
                    ...
                    Rateschecker.CheckRates(memoryMappedFileExchangeRate);
                }
                if (Math.Abs(timeWeather.Hour - DateTime.Now.Hour) == 0)
                {
                    ...
                    WeatherChecker.CheckWeather(memoryMappedFileWeatherForecast);
                }
                if (Math.Abs(timeStockePrice.Minute - DateTime.Now.Minute) == 0)
                {
                    ...
                    StockPricesChecker.CheckStockPrices(memoryMappedFileStockPrices);
                    var isLocked = Semaphore.TryOpenExisting("SynchronizationSemaphore", out _pool);
                    if (isLocked)
                        _pool.Release();
                }
            }
