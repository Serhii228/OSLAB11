    ...
    public partial class MainWindow : Window
    {
        ...
        private void UpdateTimer_Tick(object sender, EventArgs e)
        {
            if (checkWeatherForecast)
                OpenExistingMMF("MMF_WeatherForecast", lbData);
            if (checkExchangeRates)
                OpenExistingMMF("MMF_ExchangeRate", lbRate);
            if (checkStockPrices)
                OpenExistingMMF("MMF_StockPrices", lbStock);
        }
        private void OpenExistingMMF(string MMF_Name, System.Windows.Controls.Label label)
        {
            ...
            using (BinaryReader binReader = new BinaryReader(stream))
                label.Content = new string(binReader.ReadChars((int)stream.Length)).Trim('\0');
        }
